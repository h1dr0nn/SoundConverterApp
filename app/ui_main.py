...
class MainWindow(QWidget):
    ...
    def _export_audio(self) -> None:
        if not self._dependencies_ready:
            ready, message = self.converter.dependencies_ready()
            if not ready:
                QMessageBox.critical(self, "Thiếu thư viện", message)
                return
    ...
    def _check_dependencies(self) -> bool:
        ready, message = self.converter.dependencies_ready()
        if not ready:
            self.status_label.setText("Thiếu thư viện")
            QMessageBox.critical(self, "Thiếu thư viện", message)
    ...
    def _set_controls_enabled(self, enabled: bool) -> None:
        ...
        self.clear_button.setEnabled(enabled and self.input_file is not None)
